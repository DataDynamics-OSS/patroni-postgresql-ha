...
접속하면, 그 뒤에서 여러 부품이 협력해 항상 살아있는 리더에게 요청을 전달합니다.
...
flowchart TD
    ...
    VIP --> HAP["HAProxy<br/>요청을 리더에게 길안내"]
...
- **PostgreSQL(Patroni) 노드는 자기들끼리 투표하지 않습니다.** 리더 결정은 etcd가 하므로
  DB는 **2대(리더 1 + 복제본 1)만으로도 안전하게** 자동 페일오버됩니다. 다만 DB가 2대면
